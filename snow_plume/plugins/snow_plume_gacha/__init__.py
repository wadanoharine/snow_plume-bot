...
@gacha_cmd.got("mode", prompt="请选择卡池类型: 1: 原神,\n2: 明日方舟单up,\n3:明日方舟双up\n也可以输入0: 自定义以diy卡池参数")
@gacha_cmd.got("n", prompt="请输入抽数")
@gacha_cmd.got("e", prompt="请输入期望抽到up角色的数量，如有多up请以逗号分隔")
async def handling(mode: str=ArgStr("mode"), n: str=ArgStr("n"), e: str=ArgStr("e")):
    ...
    try:
        ...
        await gacha_cmd.reject_arg("e","期望结果输入错误！请输入以逗号为分隔的正整数")
...
async def gacha_calc(pool: gacha.step, n: int, e: list):
    if max(e)>10:
        ...
        result = pool.calc(n,e) if len(e)==1 else pool.smlt(n,e)
        return f"{n}抽达到期望结果{e}的概率是{result}"
